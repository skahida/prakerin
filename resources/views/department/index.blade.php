...
@section('title', 'Manajemen Jurusan — Tera Prakerin')
            ...
            <div>
                <h2 class="text-4xl font-black text-slate-800 tracking-tight flex items-center gap-3">
                    <span class="bg-teal-600 text-white p-3 rounded-2xl shadow-xl shadow-teal-200">
                        <span class="material-icons-round block">account_tree</span>
                    </span>
                    Jurusan
                </h2>
                <p class="text-slate-500 font-medium mt-2 uppercase text-xs tracking-[0.2em]">
                    Manajemen Data Jurusan / Program Keahlian
                </p>
            </div>
        </div>
            ...
            <div class="px-8 py-6 border-b border-slate-50 flex items-center justify-between">
                <div>
                    <h3 class="text-lg font-black text-slate-800 tracking-tight">
                        {{ isset($department) ? 'Edit Jurusan' : 'Tambah Jurusan Baru' }}
                    </h3>
                    <p class="text-xs text-slate-400 font-medium mt-1 uppercase tracking-wider">
                        {{ isset($department) ? 'Perbarui data jurusan yang dipilih' : 'Isi data jurusan dengan lengkap' }}
                    </p>
                </div>
                @if (isset($department))
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="code"
                                class="w-full rounded-2xl border-slate-200 bg-slate-50 text-sm font-medium focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all px-5 py-3.5 {{ isset($department) ? 'bg-slate-100 cursor-not-allowed opacity-70' : '' }}"
                                value="{{ old('code', isset($department) ? $department->code : '') }}"
                                placeholder="Contoh: RPL" autofocus {{ isset($department) ? 'readonly' : '' }}>
                            @error('code')
                    ...
                    <div class="mt-8 flex flex-col sm:flex-row gap-3">
                        <button type="submit"
                            ...
                            <span class="material-icons-round text-lg">
                                {{ isset($department) ? 'edit' : 'save' }}
                            </span>
                            {{ isset($department) ? 'Perbarui Data' : 'Simpan Jurusan' }}
                        </button>
                ...
                <form method="GET" class="flex flex-col sm:flex-row gap-3">
                    <div class="flex-1 relative">
                        ...
                        <input type="text" name="search"
                            ...
                            placeholder="Cari nama atau kode jurusan..." value="{{ $search ?? '' }}">
                    </div>
                                    ...
                                    <tr class="group hover:bg-slate-50/50 transition-colors">
                                        <td class="px-8 py-6">
                                            <span
                                                ...
                                                {{ sprintf('%02d', $departments->firstItem() + $i) }}
                                            </span>
                                        </td>
                                        ...
                                        <td class="px-8 py-6">
                                            <div class="flex justify-end gap-3">
                                                <a href="{{ route('department.edit', $item->code) }}"
                                                ...
                                                <button type="button"
                                                    ...
                                                    data-id="{{ $item->code }}" title="Arsipkan">
                    ...
                    @if ($departments->hasPages())
                        <div
                            ...
                            <div class="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">
                                Menampilkan
                                <span class="text-slate-800">{{ $departments->firstItem() }}</span> -
                                <span class="text-slate-800">{{ $departments->lastItem() }}</span> dari
                                <span class="text-slate-800">{{ $departments->total() }}</span> data
                            </div>

                            <div class="flex items-center gap-2">
                                <a href="{{ $departments->previousPageUrl() ?? '#' }}"
                                    class="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-slate-200 text-slate-400 hover:text-teal-600 hover:border-teal-600 transition-all shadow-sm {{ $departments->onFirstPage() ? 'opacity-40 pointer-events-none' : '' }}">
                                ...
                                <a href="{{ $departments->nextPageUrl() ?? '#' }}"
                                    class="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-slate-200 text-slate-400 hover:text-teal-600 hover:border-teal-600 transition-all shadow-sm {{ !$departments->hasMorePages() ? 'opacity-40 pointer-events-none' : '' }}">
                ...
                @else
                    ...
                    <div class="text-center py-24">
                        <div
                            ...
                            <span class="material-icons-round text-6xl">account_tree</span>
                        </div>
                        ...
                        <p class="text-slate-400 font-medium max-w-xs mx-auto mt-2">
                            Belum ada data jurusan yang terdaftar dalam sistem.
                        </p>
                    </div>
                @endif
            </div>
